...
function makeTsWrapper() {
  ...
  function toEmType(type: string) {
    ...
    throw new Error(`AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA unknown parameter type ${type}`);
  }
  ...
  function toEm(p: string | FuncParam) {
    ...
    throw new Error(`BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB unknown parameter type ${JSON.stringify(p)}`);
  }
